...
AUTHOR_DURATION = {"SPOC": 120, "QLP": 1800}
...
def HR_scatter(min_p_orb = 0, max_p_orb = 10, 
    ...
    plt.savefig('TESSmag_HR_scatter_color' + str(min_p_orb) + '.' + str(max_p_orb) + '.' + str(min_mass) + '.' +
                str(max_mass) + '.' + str(min_rad) + '.' + str(max_rad) + '.' + str(brightness) +'.png', bbox_inches="tight")
...
def format_fits_header_info_all_headers(star, author_input, i):
    ...
    t_start = info["TSTART"].values[0] if "TSTART" in info else ""
    t_stop = info["TSTOP"].values[0] if "TSTOP" in info else ""
    ...
    mh = info["MH"].values[0] if "MH" in info else ""
    rad = round(info["RADIUS"].values[0], 2) if "RADIUS" in info else ""
    t_mag = info["TESSMAG"].values[0] if "TESSMAG" in info else ""
...
def get_pipeline_dict_values(star, author_input, i):
    ...
    return stellar_rot_period, max_power
...
def to_single_row_df(star, author_input, i):
    ...
    pipe_p_rot, pipe_max_pow = get_pipeline_dict_values(star, author_input, i)
...
def write_df_to_csv(star, author_input, i, version):
    ...
    if not os.path.exists(outdir):
        os.mkdir(outdir)
        filename = outdir + star + "_" + author_input + "_" + str(sector) + f"_{version}.csv"
...
def plot_vetting_mosaic(star, author_input, i, sr, version):
    ...
    if not os.path.exists(savedir):
        ...
        fig.savefig(savedir + star + "_" + author_input + "_" + str(sector) + f"_{version}.png", bbox_inches='tight')
    else:    
        hdr5 = get_fits_header_info_all_headers(star, author_input, i)
        sector = hdr5["SECTOR"].values[0]
        fig.savefig(savedir + star + "_" + author_input + "_" + str(sector) + f"_{version}.png", bbox_inches='tight')
...
def main2(author_input, version):
    ...
    with open(path + author_input + "_mag_check.txt", 'w') as f:
                ...
                if not stellar_magnitude_check(star):
                    ...
                    if (len(sr) < 1):
                        ...
                        for i in range(len(sr)):
                            ...
                            f.write(f"{star}, 2\n")
                            print(f"2: {star} lightcurve {i + 1}/{len(sr)} passed pipeline.")
        ...
        print("Final standings: " + str(passed_count) + " passed out of " + str(total_count) + " stars, of which " + str(nan_count) + " are nan's, " + str(dim_count) + " were too dim, and " + str(no_lc_count) + " had no lc.")                  
